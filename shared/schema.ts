...
// Operator Applications - Track tier application and verification status
export const APPLICATION_STATUSES = ["pending", "under_review", "approved", "rejected", "additional_info_required"] as const;
...
export const operatorApplications = pgTable("operator_applications", {
  ...
  uniqueUserTier: unique().on(table.userId, table.tier)
}));

export const insertOperatorApplicationSchema = createInsertSchema(operatorApplications).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
// Application Documents - Store uploaded verification documents
export const DOCUMENT_TYPES = ["drivers_license", "business_license", "insurance_certificate", "vehicle_registration", "equipment_photo", "certification", "identity_document"] as const;
...
export const VERIFICATION_TOKEN_TYPES = ["email", "sms", "password_reset"] as const;
export type VerificationTokenType = typeof VERIFICATION_TOKEN_TYPES[number];
export const verificationTokenTypeSchema = z.enum(VERIFICATION_TOKEN_TYPES);
